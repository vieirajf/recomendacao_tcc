...
public class UsuarioController {
	...
	public String adicionar(@Valid Usuario usuario, BindingResult result, RedirectAttributes redirectAttributes) {
		if (result.hasErrors()) {
			return "usuario/adicionar";
		}
		
		if(usuarioService.getUsuarioByLogin(usuario.getLogin()) != null) {
			result.rejectValue("login", "Repeat.usuario.login", "Já existe um usuario com esse login");
			return "usuario/adicionar";
		}
		...
		redirectAttributes.addFlashAttribute("info", "Usuario adicionado com sucesso.");
